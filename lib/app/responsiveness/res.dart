...
class McGyver {
  ...
  static double _fixedWidth = 0;

  /// Defined in pixels !!
  static double _fixedHeight = 0;

  /// Defined in pixels !!
  static const bool _isFullScreenApp = false;

  /// Define whether app is a fullscreen app [true] or not [false] !!
  ...
  static Orientation setScaleRatioBasedOnDeviceOrientation(
    ...
    if (MediaQuery.of(context).orientation == Orientation.portrait) {
      ...
      /// Ration: 2 [height]
      scaleAxis = Orientation.portrait;
      ...
      /// Ration: 1 [height]
      scaleAxis = Orientation.landscape;

      /// Shortest axis == height !!
    }
    return scaleAxis;
  }
  ...
  static int rsIntW(BuildContext context, double scaleValue) {
    ...
    final origVal = McGyver.rsDoubleW(context, scaleValue);
    return McGyver.roundToDecimals(origVal, 0).toInt();
  }
  ...
  static int rsIntH(BuildContext context, double scaleValue) {
    ...
    final origVal = McGyver.rsDoubleH(context, scaleValue);
    return McGyver.roundToDecimals(origVal, 0).toInt();
  }
  ...
  static double rsDoubleW(BuildContext context, double wPerc) {
    ...
    /// Device Screen Properties (dimensions etc.).
    var scrnWidth = screenSize.width.floorToDouble();

    /// Device Screen maximum Width (in pixels).

    McGyver.setScaleRatioBasedOnDeviceOrientation(context);

    ///* Set Scale-Ratio based on device orientation.

    var rsWidth = 0.0;

    ///* OUTPUT: 'rsWidth' == Ratio-Scaled Width (in pixels)
    if (scrnWidth == _fixedWidth) {
      ...
      var wPxlsInpVal = (wPerc / 100) * _fixedWidth;

      /// 'wPxlsInpVal' == Width in Pixels of Input Value.
  ...
  static double rsDoubleH(BuildContext context, double hPerc) {
    ...
    var scrnSize = MediaQuery.of(context).size;

    /// Device Screen Properties (dimensions etc.).
    var scrnHeight = scrnSize.height.floorToDouble();

    /// Device Screen maximum Height (in pixels).
    var statsBarHeight = MediaQuery.of(context).padding.top;
    ...
    ///* OUTPUT: 'rsHeight' == Ratio-Scaled Height (in pixels)
    if (scrnHeight == _fixedHeight) {
      ...
      var hPxlsStatsBar = (hPercStatsBar / 100) * hPxlsInpVal;

      /// 'hPxlsStatsBar' == Height in Pixels of Status Bar.
      ...
      var hAdjStatsBarPxls = _isFullScreenApp ? 0 : hPxlsStatsBar;

      /// Set to 'zero' if FULL SCREEN APP !!
